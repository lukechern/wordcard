...
[![Version](https://img.shields.io/badge/version-4.5-blue.svg?style=for-the-badge)](https://github.com/lukechern/wordcard)
...
- **一键复制地址**: 局域网访问地址一键复制到剪贴板，操作更便捷

### 📱 前台查询卡片优化升级
- **界面美观度提升**: 优化前台查询卡片的视觉设计，界面更加精美协调
- **响应速度优化**: 提升查询卡片的响应速度，操作更加流畅自然
- **交互体验改进**: 改进用户交互逻辑，减少操作步骤，提升使用效率
- **视觉一致性**: 统一设计语言，与应用整体风格保持高度一致

### 🔄 智能操作模式切换
- **互斥开关设计**: 手机操作和电脑操作采用互斥开关设计，同时只能开启一种模式
- **状态持久化**: 用户选择的操作模式会被智能保存，切换标签页后状态保持不变
- **内容动态显示**: 手机操作开关控制对应功能区域的显示和隐藏，界面更加简洁
- **智能状态同步**: 开启一种操作模式时，另一种模式自动关闭，避免功能冲突

### 📊 实时数据统计显示
- **动态单词计数**: 手机端标题显示"WordCard 数据管理"，副标题实时显示"共X条单词记录"
- **PC端数据同步**: PC端Web界面显示"APP中当前有X条单词记录"，与手机端数据保持同步
- **导入后自动刷新**: 数据导入成功后，单词数量自动更新，无需手动刷新
- **版本信息动态化**: 帮助页面版本号从硬编码改为动态读取，与应用版本自动同步

### 🎨 用户体验全面提升
- **操作反馈优化**: PC端导入成功后自动清空输入字段，避免重复操作
- **信息层次清晰**: 主标题和数据统计分离显示，信息层次更加清晰
- **跨平台一致性**: PC端和手机端界面风格统一，操作逻辑一致
- **智能错误处理**: 完善的错误处理机制，网络异常时提供友好的错误提示
...
### 🔍 智能搜索功能全面升级
- **实时搜索体验**: 在单词本页面新增智能搜索功能，点击放大镜图标即可进入搜索模式
- **无缝界面切换**: 标题栏与搜索框之间平滑切换，高度保持一致，消除视觉跳动
- **专用搜索光标**: 创建专门的左对齐虚拟光标组件，与首页居中光标区分，确保搜索体验的一致性
- **智能键盘控制**: 右下角动态按钮，键盘展开时显示向下箭头（收起），收起时显示向上箭头（展开）
- **优雅按钮设计**: 搜索控制按钮图标加大50%，移除透明背景，提供清晰的视觉反馈

### 🎯 用户交互体验优化
- **智能位置调整**: 键盘控制按钮根据键盘状态智能调整位置
  - 键盘展开时：位于键盘右上角，便于快速收起
  - 键盘收起时：移动到底部导航栏上方，方便重新展开
- **统一控制逻辑**: 移除搜索框点击弹出键盘的冲突逻辑，改为通过右下角按钮统一控制
- **实时搜索反馈**: 输入字符即时显示匹配结果，支持模糊搜索和英文字母过滤
- **状态保持优化**: 收起键盘不退出搜索模式，方便用户查看被遮挡的内容

### 🔧 技术架构创新
- **组件化搜索栏**: 创建独立的`SearchBarComponent_7ree`组件，支持标题栏和搜索框无缝切换
- **专用光标组件**: 新增`SearchCustomCursor_7ree`组件，专门处理搜索框的左对齐光标需求
- **状态管理优化**: 在`PaginationState_7ree`中新增搜索状态管理，支持搜索查询和模式切换
- **数据层搜索**: 在`DataManager_7ree`中实现搜索功能，利用现有数据库查询能力

### 🎨 界面设计精进
- **固定标题栏高度**: 通过72.dp固定高度确保标题模式和搜索模式下的视觉一致性
- **圆角样式统一**: 搜索框圆角16.dp与首页搜索框保持一致的设计语言
- **动态图标切换**: 键盘控制按钮图标根据状态智能切换，提供直观的操作反馈
- **精确位置控制**: 按钮位置经过精确计算，确保在不同状态下都有最佳的操作体验

### 📱 用户体验提升
- 搜索功能与单词本完美集成，提供流畅的查找体验
- 智能键盘控制让用户可以灵活查看内容和输入文字
- 实时搜索结果让用户快速找到目标单词
- 统一的设计语言提供一致的视觉体验
...
### 🔐 API 密钥安全存储
- **加密存储技术**: 全新实现的 API 密钥加密存储系统，使用 Android Keystore 和 AES 加密算法
- **安全迁移机制**: 自动检测并迁移旧版本的明文存储密钥到加密存储，确保数据安全
- **完整性验证**: 内置存储完整性验证机制，确保密钥数据的可靠性和安全性
- **透明升级**: 用户无感知的安全升级，现有配置自动迁移到安全存储
- **多重保护**: 结合设备硬件安全模块，提供企业级的密钥保护

### 🔊 智能自动朗读功能
- **单词查询自动朗读**: 新增通用设置选项，开启后单词查询完成时自动朗读单词发音
- **拼写练习自动朗读**: 新增通用设置选项，开启后拼写练习卡片打开时自动朗读目标单词
- **桌面小组件支持**: 桌面小组件查询完成后同样支持自动朗读功能，与应用内体验保持一致
- **智能触发机制**: 自动朗读功能智能检测 TTS 准备状态，确保朗读功能的稳定性
- **用户友好配置**: 在通用设置页面提供两个独立的开关，用户可根据需求灵活配置

### 🎨 界面设计统一化
- **背景色标准化**: 统一了通用设置、数据管理和帮助页面的卡片背景色，采用一致的浅灰色设计
- **视觉一致性**: 所有设置页面现在使用相同的圆角和内边距设计，提升整体视觉协调性
- **布局优化**: 优化了键盘设置选项的布局，将两个选项放置在同一行，节省垂直空间
- **间距调整**: 精细调整了各个设置区域之间的间距，确保页面布局更加紧凑合理

### 🔧 技术架构升级
- **安全存储架构**: 新增 `ApiKeySecureStorage_7ree` 组件，专门处理敏感信息的安全存储
- **配置系统重构**: 重构了 `AppConfigManager_7ree`，支持安全存储和明文存储的无缝切换
- **组件参数传递**: 优化了拼写练习组件的参数传递机制，支持 TTS 功能的完整集成
- **错误处理增强**: 加强了安全存储相关的错误处理和异常恢复机制

### 🎯 用户体验提升
- API 密钥等敏感信息得到企业级安全保护，用户可以放心使用
- 自动朗读功能让学习体验更加智能化和便捷化
- 统一的界面设计语言提供更加专业和一致的视觉体验
- 灵活的配置选项让用户可以根据个人喜好定制学习体验
